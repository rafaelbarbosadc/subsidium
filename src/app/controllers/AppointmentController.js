...
class AppointmentController {
  ...
  async store (req, res) {
    ...
    const date_end = moment(date)
      .add(duration_int, 'hours')
      .format()
  ...
  appointment.id
                  ...
                  const people_reserved = await Appointment.sum(
                    'people_quantity',
                    {
                      where: {
                        provider_id: provider,
                  ...
                  const table_quantity = restaurant.table_quantity
                  const table_available = Math.trunc(
                    table_quantity - people_reserved / 4
                  )
                  const table_requested = Math.ceil(people_quantity / 4)
  ...
  appointment.id
          ...
          req.flash(
            'error',
            'Desculpe, o restaurante já está lotado no horário escolhido!'
          )
        }
      }
    }
